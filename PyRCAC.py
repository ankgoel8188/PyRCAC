...
class RCAC:
    ...
    def __init__(self, Nc = 4, Rz = 1, Ru = 0e+1, RegZ = 1, FF = 0, Integrator = 1, FIR = 0, ContType = "dense", R0 = 1e5, Lambda = 0.9995):
        ...
        self.R0 = R0
        self.Lambda = Lambda
    def simulate(self, System, Filter, Step = 500 , Refrence_Signal = 'Step_Input'):
        ...
        self.CalculateRegSize(System)
        
        """3rd Step: Initialize variables for the simulation"""
        x, u, y0, z, theta = self.Simulation_Initializer(System)
        ...
        for kk in range(self.Step):
            ...
            if kk == 0:
                ...
                z[:,kk] = y0[:,kk] - ref
                u[:,kk], theta[:,kk] = self.RCAC_Control(kk, np.zeros((System.lu,1)), 0*z[:,kk], 0*z[:,kk], ref, Filter, System) 
            else:
                x[:,kk] = (np.matmul(System.A, x[:,kk-1].reshape(System.lx,1)) + np.matmul(System.B, u[:,kk-1].reshape(System.lu,1))).reshape(System.lx,)
                y0[:,kk] = np.matmul(System.C, x[:,kk].reshape(System.lx,1)).reshape(System.ly,)
                z[:,kk] = y0[:,kk] - ref
                u[:,kk], theta[:,kk] = self.RCAC_Control(kk, u[:,kk-1], z[:,kk-1], 0*z[:,kk-1], ref, Filter, System)
    ...
    def CalculateRegSize(self, System):
        if self.Nc == 0:
    ...
    def Simulation_Initializer(self, System):
